...
__global__ void medianFilterSharedKernel(unsigned char *inputImageKernel, unsigned char *outputImagekernel, int imageWidth, int imageHeight,int tile_size,int sz)
{
  ...
  int n=sz*2+1;
  // unsigned char filterVector[n*n];
  int r[51], g[51], b[51];


  if(x<imageWidth && y<imageHeight){

    __shared__ unsigned char sharedmem_red[(tile_size+sz)][(tile_size+sz)];  //initialize shared memory
    __shared__ unsigned char sharedmem_green[(tile_size+sz)][(tile_size+sz)];  //initialize shared memory
    __shared__ unsigned char sharedmem_blue[(tile_size+sz)][(tile_size+sz)];  //initialize shared memory
    for(int i=0;i<(tile_size+sz);i++){
      for(int j=0;j<(tile_size+sz);j++){
        int idxI = (y+sz) * imageWidth + (x+sz);
        idxI=idxI-((imageWidth)*(sz)+(sz));
        sharedmem_red[threadIdx.x+i][threadIdx.y+j]=(int)inputImageKernel[CHANNELS*idxI];
        sharedmem_green[threadIdx.x+i][threadIdx.y+j]=(int)inputImageKernel[CHANNELS*idxI+1];
        sharedmem_blue[threadIdx.x+i][threadIdx.y+j]=(int)inputImageKernel[CHANNELS*idxI+2];
        idxI++;
      }
    }
    __syncthreads();
    int i,j,idx;
    for(i=0;i<(tile_size+sz);i++){
      for(j=0;j<(tile_size+sz);j++){
        idx=i*(tile_size+sz)+j;
        r[idx]=sharedmem_red[threadIdx.x+i][threadIdx.y+j];
        g[idx]=sharedmem_red[threadIdx.x+i][threadIdx.y+j];
        b[idx]=sharedmem_red[threadIdx.x+i][threadIdx.y+j];
      }
    }
    ...
    int filIdx = (y+sz) * imageWidth + (x+sz);
    outputImagekernel[CHANNELS*filIdx]=(unsigned char)r[(n*n)/2];
    outputImagekernel[CHANNELS*filIdx+1]=(unsigned char)g[(n*n)/2];
    outputImagekernel[CHANNELS*filIdx+2]=(unsigned char)b[(n*n)/2];
...
__global__ void colCon(unsigned char * outImg, unsigned char *inImage, int width, int height, int pwidth, int sz) {
  ...
  int n=sz*2+1;
  const unsigned char filterVector[n*n];
//int sa=n*n;
  int r[51], g[51], b[51];
  //int *arr;
  if(x<width && y<height){
...
int main(int argc, char **argv){
    ...
    // cudaFree(out);
    cudaEventRecord(start);
    medianFilterSharedKernel<<<gridSize,blockSize>>>(out,in,padedImg->x,padedImg->y,4,sz);
    cudaEventRecord(stop);
